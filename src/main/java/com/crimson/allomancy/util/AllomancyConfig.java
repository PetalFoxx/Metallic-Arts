...
public class AllomancyConfig {
    ...
    public static class CommonConfig {
        ...
        private static List<String> default_whitelist() {
            ...
            defaultList.add(Items.LANTERN.getRegistryName().toString());

            defaultList.add(Blocks.ANVIL.getRegistryName().toString());
            defaultList.add(Blocks.IRON_TRAPDOOR.getRegistryName().toString());
            defaultList.add(Blocks.ACACIA_DOOR.getRegistryName().toString());
            defaultList.add(Blocks.OAK_DOOR.getRegistryName().toString());
            defaultList.add(Blocks.DARK_OAK_DOOR.getRegistryName().toString());
            defaultList.add(Blocks.SPRUCE_DOOR.getRegistryName().toString());
            ...
            defaultList.add(Blocks.LANTERN.getRegistryName().toString());
            ...
            for (int i = 0; i < Registry.metals.length; i++) {
                defaultList.add("allomancy:" + Registry.metals[i] + "_flakes");
            }
            
            for (int i = 0; i < Registry.metals.length; i++) {
                defaultList.add("allomancy:" + Registry.metals[i] + "_nugget");
            }
    ...
    public enum MOB {
    	...
    	private int throwDiff;
    	  
    	    // enum constructor called separately for each 
    	    // constant 
    	    private MOB(int emotionDiff, int throwDiff) 
    	    { 
    	        this.emotionDiff = emotionDiff;
    	        this.throwDiff = throwDiff;
    	    } 
    	    
    	    public int getStrength(INTERACTIONTYPE interaction) {
    	    	if(interaction == INTERACTIONTYPE.EMOTE)
    	    		return emotionDiff;
    	    	else
    	    		return throwDiff;
    	    }
