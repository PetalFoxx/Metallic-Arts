...
public class MetalSelectScreen extends Screen {
    ...
    public void render(int mx, int my, float partialTicks) {
        ...
        for (int seg = 0; seg < segments; seg++) {
        	//HERE
        	//if(cap.getAllomancyPower() == seg || cap.getAllomancyPower() == 8) {
            ...
            if (cap.getMetalFlaring((seg + 4) % 8))
            {
            	r = 1.0f;
            	g = gs;
            	b = gs;
            } else if (cap.getMetalBurning((seg + 4) % 8)) {
            	r = 0.7f;
            	g = 0.3f + gs;
            	b = 0.3f + gs;
            } else if (!cap.canBurn((seg + 4) % 8)) {
            	r = 0.75f;
            	g = 0.75f + gs;
            	b = 0.75f + gs;
            } else {
            ...
            for (float i = degPer; i >= 0; i--) {
                ...
                if (i == (int) (degPer / 2)) {
                    ...
                    stringPositions.add(
                            new int[]{seg, (int) xp, (int) yp, (cap.canBurn((seg + 4) % 8) ? '7' : 'f')}); // Mark unused ones as disabled
                }
